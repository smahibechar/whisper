...
def clear():
      if os.name=='nt':
          os.system('cls && title [NEW] ZER0EYE - FULL KEY GENERAT0R BY Whisper ')
      else :
          os.system('clear')
#------------#
def strounga(x,y):
   ...
   return omri
#------------#
logo=""" _    _ _   _ _____ ___________ ___________ \n| |  | | | | |_   _/  ___| ___ \  ___| ___ \ \n| |  | | |_| | | | \ `--.| |_/ / |__ | |_/ / \n| |/\| |  _  | | |  `--. \  __/|  __||    /  \n\  /\  / | | |_| |_/\__/ / |   | |___| |\ \ \n \/  \/\_| |_/\___/\____/\_|   \____/\_| \_| \n
                                     """
#------------#
#Bank Names#
...
class ccSama(object):
  ...
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"""%(blid,lasfar,lazra9,lasfar,lazra9,lasfar,la7mar,lazra9,lasfar,la7mar,lazra9,lasfar,la7mar,lazra9,lasfar))
    def gen1(self ,Sa7by):#Sa7by Kol Chy Wadha7 W jawna Bahy Brcha xDD...
      ...
      with open('Whisper-results/ccgenerated.txt','a') as escnoor:
      ...
      print(" %s[%s%s Saved In 'Whisper-results/ccgenerated.txt'%s]%s"%(la7mar,lasfar,mouchi,la7mar,labyadh))
...
class ghrab(object):
    def __init__(self):
        ...
        for ip in ip_range:
            xXx=xXx+1
            with open('Whisper-results/Ipgenerated.txt', 'a') as xX:
                xX.write(str(ip) + ':' + str(PRoxYPort) + '\n')
        print("%s[%s%s Ip Saved In 'Whisper-results/Ipgenerated.txt'%s]%s"%(la7mar,lasfar,xXx,la7mar,labyadh))
        so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
        if so.upper() in ['Y','YES','AY']:
            z4ck()
        else:
...
class z4ck(object):
  ...
  def __init__(self):
      ...
      print(strounga(xnumx+5,'Back'))
      print(strounga(xnumx+6,'Delete a config'))
      print(strounga(xnumx+7,'About'))
      baka=input('\033[91m[\033[93mWhisper>\033[91m]\033[00m ')
      while baka=='':
        baka=input('\033[91m[\033[93mWhisper>\033[91m]\033[00m ')
      baka=int(baka)
      while baka <1 or baka >(xnumx+7):
         baka=int(input('\033[91m[\033[93mWhisper>\033[91m]\033[00m '))
      if baka<=xnumx:
...
#1,2,3 Go!
  ...
  def x1x(self,x):
   ...
   return x444
  def phnnm(self):
    if not os.path.isdir('Whisper-results/phonz'):
     os.mkdir('Whisper-results/phonz')
    print("""
        Helper:
    ...
    for i in range(Hwmch):
      ...
      ousm='+'+self.x1x(bakas)
      print(ousm)
      with open('Whisper-results/phonz/C0de_%s.txt'%(str(intcd)),'a') as L:
        L.write('%s\n'%ousm)
    print(" %s[%s%s number phone Saved In 'Whisper-results/phonz/C0de_%s.txt'%s]%s"%(la7mar,lasfar,Hwmch,str(intcd),la7mar,labyadh))
    so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
    if so.upper() in ['Y','YES','AY']:
           z4ck()
    else:
...
#fbtokengen
  def genetok(self):
   ...
   for az in range(int(zz)):
    ...
    print(escnoor)
    with open('Whisper-results/tokens.txt','a') as ttt:
      ttt.write(str(escnoor)+'\n')
   print(" %s[%s%s Token Saved In 'Whisper-results/tokens.txt'%s]%s"%(la7mar,lasfar,zz,la7mar,labyadh))
   so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
   if so.upper() in ['Y','YES','AY']:
            z4ck()
   else:
...
#Key creator 
  ...
  def delmyconf(self):
    ...
    try:
      ...
      sys.exit(input('%s[%s.config.ini NO FOUND !!%s]%s'%(la7mar,lasfar,la7mar,labyadh)))
  ...
  def seemydick(self):
   print("""{r}
Whisper""".format(r=la7mar,y=lasfar,rr=ramadi))
#------------#    
try:
  if not os.path.isdir('Whisper-results'):
    os.mkdir('Whisper-results')
  if not os.path.isfile('.config.ini')or len(open('.config.ini','r').read())<1:
      ...
      open('.config.ini','w').write('#Whisper\n')
